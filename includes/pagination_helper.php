 ...
 * NOTE: Uses 'pg' parameter to avoid conflict with router's 'page' parameter.
 */
...
/**
 * Render Bootstrap 5 pagination controls with glassmorphism styling
 * 
 * @param array $pagination The pagination array from paginate()
 * @param string $base_url The base URL for pagination links (without pg param)
...
function render_pagination($pagination, $base_url = '', $max_links = 5) {
    ...
    // Previous button
    if ($current_page > 1) {
        $html .= '<li class="page-item"><a class="page-link" href="' . htmlspecialchars($base_url . $separator . 'pg=' . ($current_page - 1)) . '">&laquo;</a></li>';
    } else {
    ...
    // First page + ellipsis
    if ($start > 1) {
        $html .= '<li class="page-item"><a class="page-link" href="' . htmlspecialchars($base_url . $separator . 'pg=1') . '">1</a></li>';
        if ($start > 2) {
    ...
    for ($i = $start; $i <= $end; $i++) {
        if ($i == $current_page) {
            ...
            $html .= '<li class="page-item"><a class="page-link" href="' . htmlspecialchars($base_url . $separator . 'pg=' . $i) . '">' . $i . '</a></li>';
        }
    }
    ...
    if ($end < $total_pages) {
        ...
        $html .= '<li class="page-item"><a class="page-link" href="' . htmlspecialchars($base_url . $separator . 'pg=' . $total_pages) . '">' . $total_pages . '</a></li>';
    }
    
    // Next button
    if ($current_page < $total_pages) {
        $html .= '<li class="page-item"><a class="page-link" href="' . htmlspecialchars($base_url . $separator . 'pg=' . ($current_page + 1)) . '">&raquo;</a></li>';
    } else {
...
/**
 * Build base URL preserving current GET params except 'pg' (pagination)
 * 
 * @return string The base URL with existing query params
 */
function get_pagination_base_url() {
    $params = $_GET;
    unset($params['pg']); // Remove pagination param
